# [ThreedsSdkAndroidDoc](https://sdkpay.github.io/3dsSdkAndroidDoc/)

#### [Сценарии работы с SDK](https://sdkpay.github.io/3dsSdkAndroidDoc/sdk_scenario) | [Сущности и классы](https://sdkpay.github.io/3dsSdkAndroidDoc/sdk_classes) | [Актуальная версия SDK](https://sdkpay.github.io/3dsSdkAndroidDoc/sdk_version) | [Различия версий SDK](https://sdkpay.github.io/3dsSdkAndroidDoc/sdk_differences)

<br>

# Актуальная версия 1.1.3

## Версия 1.1

### 1.1.3

- Оптимизация работы SDK
- Улучшение логирования

### 1.1.2

- Оптимизация работы, облегчение SDK, улучшение логирования для отслеживания проблем в работе SDK

### 1.1.1

- Добавлен статус неуспешной оплаты в сallback для мерчанта

### 1.1.0

- Упрощен SDK flow для упрощения интеграции в приложение мерчанта

## Версия 1.0

### 1.0.2

- Добавлено логирование для отслеживания проблем в работе SDK
- Устранение ошибок

### 1.0.0

- Добавлен базовый функционал
- Релиз первой версии SDK